...
//python                    assert(buffer.size()<=1);// performance assert!
...
typedef  EWAHBoolArray<uword32> bitset;
...
class Bitset {
	public:
		...
		Bitset():name("default") {}
		Bitset(PyObject *iname) { setName(iname); }
		Bitset(string iname):name(iname) {}
		Bitset(string iname, bitset ibs):name(iname), bs(ibs) {}
		void set(int i) { bs.set(i); }
		void setName(PyObject *iname) { name = PyString_AsString(iname); }
		PyObject* __str__() {
			ostringstream s;
  		vector<ulong> ids;
		  ids.clear();
		  ...
		  for(vector<ulong>::const_iterator i = ids.begin();
 			    i != ids.end(); ++i) {
      		if(i != ids.begin()) s << ", ";
		      s << name << "/" << *i;
  		}
		...
		Bitset merge(Bitset *x) {
			bitset mbs;
			bs.rawlogicalor(x->bs, mbs);
			return Bitset(name, mbs);
		}
		bool lower(Bitset *x) {
			return name < x->name;
		}
